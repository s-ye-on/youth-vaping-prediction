...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / f"lightgbm_second_stage_tuning_{RUN_TIMESTAMP}"
...
# ============================================================
# 1차 best:
# n_estimators=300, num_leaves=15, max_depth=8,
# learning_rate=0.03, min_child_samples=40,
# reg_alpha=1, reg_lambda=10, colsample_bytree=0.8, subsample=1.0

LIGHTGBM_SECOND_STAGE_PARAM_SPACE = {
    ...
    "min_child_samples": [30, 40, 60, 80, 120],
    "reg_alpha": [0.5, 1, 2, 3],
    "reg_lambda": [5, 10, 15, 20, 30],
}
...
def write_csv(path: Path, rows: list[dict]) -> None:
    if not rows:
        raise ValueError("저장할 rows가 없습니다.")
...
def run_second_stage_tuning(
    ...
    print("\n" + "=" * 80)
    print("LightGBM 2차 튜닝 시작")
    print("=" * 80)

    while trial < N_TRIALS:
        params = sample_params(LIGHTGBM_SECOND_STAGE_PARAM_SPACE, rng)
        ...
        print("\n" + "-" * 80)
        print(f"[LightGBM 2차] trial {trial}/{N_TRIALS}")
        print(params)
        print("-" * 80)
...
def main():
    ...
    tuning_df.to_csv(
        REPORTS_DIR / "lightgbm_second_stage_all_trials.csv",
        index=False,
        encoding="utf-8-sig",
    )

    tuning_df_by_pr_auc.to_csv(
        REPORTS_DIR / "lightgbm_second_stage_ranked_by_valid_pr_auc.csv",
        index=False,
        encoding="utf-8-sig",
    )

    tuning_df_by_f1.to_csv(
        REPORTS_DIR / "lightgbm_second_stage_ranked_by_valid_f1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    # PR-AUC 기준 best와 F1 기준 best가 같은 trial일 수도 있습니다.
    ...
    final_df.to_csv(
        REPORTS_DIR / "lightgbm_second_stage_final_test_results.csv",
        index=False,
        encoding="utf-8-sig",
    )

    print("\n" + "=" * 80)
    print("LightGBM 2차 튜닝 완료")
    print("=" * 80)
